...
# SPDX-FileCopyrightText: 2023 Bryan Jacobs
...
class CTAPHIDDevice:
        ...
        if self.chosen_device is not None:
            try:
                ...
                if hasattr(self.chosen_device, "close"):
                    logging.info("Closed connection to SmartCard.")
                    self.chosen_device.close()

            except Exception as e:
                logging.warning(f"Failed to close SmartCard connection: {e}")
            finally:
    ...
    def get_pcsc_device(self, channel_id: List[int]) -> Optional[CtapDevice]:
        ...
        if self.chosen_device is None:
            ...
            while time.time() < start_time + SECONDS_TO_WAIT_FOR_AUTHENTICATOR:
                logging.info("Waiting for a SmartCard...")
            ...
            raise ValueError("Could not connect to a SmartCard in time!")
    ...
    def handle_cancel(self, channel: bytes, buffer: bytes) -> Optional[bytes]:
        channel_key = self.get_channel_key(channel)
    ...
    def get_channel_key(self, channel: bytes) -> str:
        return bytes(channel).hex()

    def send_error(self, channel: bytes, error_type: int) -> None:
    ...
    def finish_receiving(self, channel: bytes) -> None:
